...
#define BSERVO_MINOR_NUMBER 100
#define SSERVO_MINOR_NUMBER 110
...
void * bservo_msg(void * arg);
void * sservo_msg(void * arg);
...
int main(int argc, char * argv[]){	
	...
	pthread_t bservo_thread,sservo_thread, wls_thread;
	void * thread_return;
	...
	pthread_create(&bservo_thread, NULL, bservo_msg, (void*)&sock);
	pthread_create(&sservo_thread, NULL, sservo_msg, (void*)&sock);
	pthread_create(&wls_thread, NULL, wls_msg, (void*)&wls_fd);
	pthread_join(bservo_thread, &thread_return);
	pthread_join(sservo_thread, &thread_return);
...
void * bservo_msg(void * arg)
{
	...
	int servo = makedev(SERVO_MAJOR_NUMBER, BSERVO_MINOR_NUMBER);
...
void * sservo_msg(void * arg)
{
	...
	int servo = makedev(SERVO_MAJOR_NUMBER, SSERVO_MINOR_NUMBER);
	mknod(SERVO_DEV_PATH, S_IFCHR | 0666, servo);
    
	servo = open(SERVO_DEV_PATH, O_RDWR);
   
	if(servo < 0){
		printf("fail to open sservo\n");
		return (void*)-1;
	}
	while(1){
		str_len=read(sock, name_msg, BUF_SIZE-1);
...
void * wls_msg(void * arg){
	...
	int led_dev = makedev(LED_MAJOR_NUMBER, LED_MINOR_NUMBER);
